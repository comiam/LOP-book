...
\textbf{Редукционный граф} терма $t\in\lmb$ - мультиорграф $G_\beta(t) = (T, \twoheadrightarrow)$, где $T = \{s\in\lmb\;|\;t\twoheadrightarrow s\}$.
...
\draw[-latex,color=black, very thick](1,0) -- (4,0);
\draw[-latex,color=black, very thick](4,0) -- (7,0);

\draw[fill=black, color=black, very thick](1,0) circle (0.05);
\draw[fill=black, color=black, very thick](4,0) circle (0.05);
\draw[fill=black, color=black, very thick](7,0) circle (0.05);

\node[scale=0.8] at (1,-0.3) {$(\lm xy.xy)uv$};
\node[scale=0.8] at (4,-0.3) {$(\lm y.uy)v$};
\node[scale=0.8] at (7,-0.3) {$uv$};
...
\node[scale=0.8] at (1,-0.7) {$(\lm x.x)((\lm x.x)x)$};
\node[scale=0.8] at (4,-0.7) {$(\lm x.x)x$};

\end{tikzpicture}

Наличие двух стрелок обусловлено тем, что мы можем получить один и тот же терм редукциями по разным переменным.\\
...
Если $s\to t$ и $t$ имеет \textbf{н.ф.}, то нормальная стратегия всегда успешно приводит $s$ к \textbf{н.ф.}.
...
0) $\churchnum{0}=\lm sz.z$;

1) $\churchnum{1}=\lm sz.sz$;

2) $\churchnum{2}=\lm sz.s(sz)$;

3) $\churchnum{3}=\lm sz.s(s(sz))$;

...

n) $\churchnum{n}=\lm sz.s^nz$.
