...
1) $\forall i=\ovl{1,n}\!:$ если $x_i=[t_i]^\lambda$, то вычеркиваем $\begin{pmatrix}x_i\\{[t_i]^\lambda}\end{pmatrix}$ столбец. \\
2) $\forall j=\ovl{1,m}\!:$ если $\exists x_i\!\!:x_i=y_j$, то мы должны вычеркнуть из матрицы $\begin{pmatrix}y_j\\u_j\end{pmatrix}$ столбец. 
...
Пусть $S$ - множество дизъюнктов. Говорят, что последовательность \\$\lot \vp 1 n=\vp$ называется \textbf{результативным выводом} дизъюнкта $\vp$ из $S$, где каждая $\vp_i\in S$ или $\vp_i=\res {\vp_j} {\vp_k},\;j<i,k<i, j\neq k$.\\
...
$\leftdok$ Пусть $S$ - выполнимо $\Rightarrow\exists\mA\!\!:\mA\vD S$. Раз на модели выполняются все дизъюнкты из S, то по теореме 28.3. $\Rightarrow\forall\vp,\psi\in S\!\!:\mA\vD\res \vp \psi$. Но т.к. $\lot \vp 1 n=\square$ - результативный вывод $\Rightarrow\mA\vD\square\Rightarrow$ получаем полный бред, т.к. $\square$ не может выполняться. Теорема доказана.
...
Основная цель наших преобразований состоит в том, чтобы привести терм к \textbf{н.ф.}. 
...
Тогда мы получим \textbf{н.ф.}, т.к $y$ мы уже ничего сделать не можем.\\
...
Если $s\to t$ и $t$ имеет \textbf{н.ф.}, то последовательность редукций, что начинается с терма $s$ и состоит в применении правила редукции внешнему(самому левому) редексу, всегда завершается и приводит терм $s$ к \textbf{н.ф.}.
...
Еще раз вспомним, что \textbf{комбинатором} называется терм без свободных переменных, т.е. все переменные связаны абстракциями. С логической точки зрения, для программирования вполне достаточно 3х комбинаторов, т.е. из них можно всё вывести. Остальные же комбинаторы используются для каких-либо иных целей. \\
\newpage
...
В чём идея следствия? Пусть, допустим, у нас существует огромный терм, но мы его можем преобразовать в такой вид, что в нём будут составляющие только того вида, что обозначены в следствии. На этом обзор $\lm$-исчисления окончен.
...
\begin{flushright}
- \textit{А в в этой логике можно описать релятивизм?}
...
Пусть $C$ - класс линейных потоков. Пусть $M\!L$ содержит модальности Приота(опр. 44.6.), тогда $M\!L$ - разделима над $C\Leftrightarrow M\!L$ - выразительно полна.\\
...
\textbf{Трёхзначная логика Лукасевича} задаётся истинностями: 
...
\begin{center}
\begin{tabular}{|c|c|c|c|llll|l|l|l|l|llll|c|c|}
\cline{1-4} \cline{9-12} \cline{17-18}
\textbf{OR} & \textbf{F} & \textbf{T} & \textbf{U} & & & & & \textbf{AND} & \textbf{F} & \textbf{T} & \textbf{U} & & & & & \textbf{NOT} & \\ \cline{1-4} \cline{9-12} \cline{17-18} 
\textbf{F} & F & T & U & & & & & \textbf{F} & F & F & F & & & & & \textbf{F} & T \\ 
\cline{1-4} \cline{9-12} \cline{17-18} 
\textbf{T} & T & T & T & & & & & \textbf{T} & F & T & U & & & & & \textbf{T} & F \\ 
\cline{1-4} \cline{9-12} \cline{17-18} 
\textbf{U} & U & T & U & & & & & \textbf{U} & F & U & U & & & & & \textbf{U} & U \\ 
\cline{1-4} \cline{9-12} \cline{17-18} 
\end{tabular}
\end{center}

Так же важно понимать, что $A\vee\lnot A\neq 1$ и $A\ampersand\lnot A\neq 0$. 
...
Таблицы истинности в данной логике задаются следующим образом. Здесь столбцы и колонки \underline{по другому поставлены}! Это для лучшего запоминания:

\begin{center}
\begin{tabular}{|c|c|c|c|llll|l|l|l|l|llll|c|c|}
\cline{1-4} \cline{9-12} \cline{17-18}
\textbf{OR} & \textbf{T} & \textbf{S} & \textbf{F} &  &  &  &  & \textbf{AND} & \textbf{T} & \textbf{S} & \textbf{F} &  &  &  &  & \textbf{NOT} &  \\ \cline{1-4} \cline{9-12} \cline{17-18} 
\textbf{T} & T & S & T &  &  &  &  & \textbf{T} & T & S & F &  &  &  &  & \textbf{T} & F \\ \cline{1-4} \cline{9-12} \cline{17-18} 
\textbf{S} & S & S & S &  &  &  &  & \textbf{S} & S & S & S &  &  &  &  & \textbf{S} & S \\ \cline{1-4} \cline{9-12} \cline{17-18} 
\textbf{F} & T & S & F &  &  &  &  & \textbf{F} & F & S & F &  &  &  &  & \textbf{F} & T \\ \cline{1-4} \cline{9-12} \cline{17-18} 
\end{tabular}
\end{center}

В 1938 г. Д.А. Бочваром была построена первая в мире логика бессмысленности в связи с проблемой разрешения логических антиномий, в первую очередь
...
\end{center}
\leavevmode\\\\\leavevmode\\\\\leavevmode\\\\\leavevmode\\\\\leavevmode\\\begin{flushright}Большим М.А., 2020\end{flushright}
